...
class VideoClip(Clip):
    ...
    def image_transform(self, image_func, apply_to=None):
        ...
        return self.transform(
            lambda get_frame, t: image_func(get_frame(t, to_np=False)), apply_to
        )
    ...
    def compose_on(self, background: Image.Image, t, backend="PIL") -> Image.Image:
        ...
        if backend == "PIL":
            ...
            return self._compose_on_cupy(background, t)
        else:
            raise ValueError("Invalid backend. Use 'PIL' or 'cupy'.")

    def _compose_on_PIL(self, background: Image.Image, t) -> Image.Image:
        if isinstance(background, np_ndarray_instance):
            background = Image.fromarray(np_get(background))
    ...
    def _compose_on_cupy(self, background: np.ndarray, t) -> np.ndarray:
        """
        ...
        background = np_convert(background)
        ...
        ct = t - self.start
        clip_frame = self.get_frame(ct, to_np=False).astype(
        ...
        if self.mask is not None:
            ...
            # and scale to 0-255.
            clip_mask = (self.mask.get_frame(ct, to_np=False) *
    ...
    def to_ImageClip(self, t=0, with_mask=True, duration=None):
        ...
        in (hour, min, sec), or as a string: '01:03:05.35'.
        """
        new_clip = ImageClip(self.get_frame(t, to_np=False),
                             is_mask=self.is_mask, duration=duration)
        if with_mask and self.mask is not None:
