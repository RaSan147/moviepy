...
from moviepy.np_handler import np, np_convert, np_get, np_ndarray, np_ndarray_instance, _np
...
class VideoClip(Clip):
    ...
    def _compose_on_cupy(self, background: np.ndarray, t) -> np.ndarray:
        ...
        background : np.ndarray
            ...
            intended to be transparent, it should be provided as an RGBA array (shape: H x W x 4)
        ...
        if y_end > y_start and x_end > x_start:
            ...
            with _np.errstate(divide='ignore', invalid='ignore'):
                out_rgb = (
                    ...
                    bg_float[..., :3] * alpha_bg[..., np.newaxis] * (1 - alpha_clip[..., np.newaxis])
                )
                out_rgb = np.where(
