...
from moviepy.np_handler import _np
    ...
    def apply(self, clip: Clip) -> Clip:
        ...
        if self.start_time is None and self.end_time is None:
            ...
            def full_volume(get_frame, t):
                frame = get_frame(t, to_np=True)
                frame = np_get(frame)
                print(isinstance(frame, _np.ndarray), "Frame is numpy:", frame.dtype)
                print("Multiplying full volume by factor:", self.factor, isinstance(frame, _np.ndarray))
                return _np.multiply(frame, self.factor)
        ...
        def volume_transform(get_frame, t):
            t = np_get(t)
            frame = get_frame(t, to_np=True)
            frame = np_get(frame)  # Ensure frame is a numpy array
            t_array = _np.asarray(t)
            factors = self._create_volume_mask(t_array)

            if nchannels > 1:
                factors = factors[:, _np.newaxis]  # Broadcast to stereo channels

            return _np.multiply(frame, factors)  # Remove dtype forcing
